...
class VPDetection(object):
    ...
    def __init__(self,
        ...
        self.clusters = None  # Stores which line index maps to what VP
        self.__tol = 1e-8  # Tolerance for floating point comparison
        self.__angle_tol = np.pi / 3  # (pi / 180 * (60 degrees)) = +/- 30 deg
        self.lines = None  # Stores the line detections internally
        ...
        self.__lines_inside_box = None
        self.__lines_outside_box = None
    ...
    def __get_edges_on_line(self, edges, x1, y1, x2, y2, dist = 6):
        points_on_line = list(self.__bresenham_line(x1, y1, x2, y2))
        edges_on_line = []
        for x, y in points_on_line:
            for xi, yi in product(range(x - dist, x + dist + 1), range(y - dist, y + dist + 1)):
                if 0 <= xi < edges.shape[1] and 0 <= yi < edges.shape[0]:
                    if edges[int(yi), int(xi)] != 0:
                        edges_on_line.append((x, y))
                        break
        return edges_on_line

    def __shorten_lines(self, edges, lines, distance = 20):
        """
        Parameters:
        -----------
        distance: int
            cut points that have distance between them that is bigger then
        """
        sub_lines = []
        for line in lines:
            edges_on_line = self.__get_edges_on_line(edges, line[0], line[1], line[2], line[3])
            sub_lines += self.__find_sub_lines_on_derivative(edges_on_line, distance)

        return np.array(sub_lines)


    def __find_sub_lines_on_derivative(self, edges_on_line, distance, single_line_length_threshold = 80):
        if not edges_on_line:
            return []
        distances = [self.__distance_between_points(p1, p2) < distance for p1, p2 in zip(edges_on_line, edges_on_line[1:] + edges_on_line[-1:])]
        lines = []
        p1, p2 = edges_on_line[0], edges_on_line[0]
        
        was_fasle_before = False
        current_line_length = 1
        for i in range(len(distances)):
            if distances[i] == False and was_fasle_before:
                continue
            elif distances[i] == False and was_fasle_before == False:
                if current_line_length > single_line_length_threshold:
                    lines.append((*p1, *p2))
            elif distances[i] == True and was_fasle_before:
                current_line_length = 1
                p1 = edges_on_line[i]
            else: # distances[i] == True and was_fasle_before == False
                current_line_length += 1
                p2 = edges_on_line[i]
            was_fasle_before = distances[i] == False
        if current_line_length > single_line_length_threshold:
            lines.append((*p1, *p2))
    ...
    def __detect_lines(self, img):
        ...
        # # with zoom
        # gaussian_size = (15, 15)
        # blurred = cv2.GaussianBlur(img_copy, gaussian_size, 2)
        # edges = cv2.Canny(blurred, 80, 100)
        # plt.imshow(edges)
        # plt.axis('off')
        # plt.show()
        # lines = cv2.HoughLines(edges, rho=0.2, theta=np.pi/180, threshold=25)  # Adjust threshold as needed
        ...
        # # plt.axis('off')
        # # plt.show()
        # # lines = cv2.HoughLines(edges, rho=0.5, theta=np.pi/360, threshold=80)  # Adjust threshold as needed
        ...
        #         x2 = int(x0 - H * (-b))
        #         y2 = int(y0 - W * (a))
        #         lines_as_xy.append([x1, y1, x2, y2])
        
        # plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))

        # # Plot the lines
        # for line in lines_as_xy:
        #     plt.plot((line[0], line[2]), (line[1], line[3]), 'r-', linewidth=1)
        # plt.axis('off')
        # plt.show()
        # lines_as_xy = self.__shorten_lines(edges, lines_as_xy, 20)
        # lines_as_xy = np.array(lines_as_xy).astype(np.float32)

        # # # Filter out the lines whose length is lower than the threshold
        # # dx = lines_as_xy[:, 2] - lines_as_xy[:, 0]
        # # dy = lines_as_xy[:, 3] - lines_as_xy[:, 1]
        # # lengths = np.sqrt(dx * dx + dy * dy)
        # # mask = lengths >= self._length_thresh
        # # lines_as_xy = lines_as_xy[mask]
        # self.lines = lines_as_xy
        # plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    ...
    def __find_vp_hypotheses_two_lines(self):
        ...
        p1 = np.column_stack((self.lines[:, :2], np.ones(N,
                                                         dtype=np.float32)))
        p2 = np.column_stack((self.lines[:, 2:], np.ones(N,
                                                         dtype=np.float32)))
    ...
    def __get_sphere_grids(self):
        ...
        combos = list(combinations(range(self.lines.shape[0]), 2))
    ...
    def set_model_lines_and_update_vps(self, kind):
        """
        Parameters
        ----------
        kind : str
        ...
        plot_lines = True
        if kind == 'in':
            self.lines = self.__lines_inside_box
        elif kind == 'out':
            self.lines = self.__lines_outside_box
        elif kind == 'all':
            self.lines = self.__all_lines
        else:
            plot_lines = False
            print(f'kind is not defined well')
        
        # plt.imshow(cv2.cvtColor(self.__img, cv2.COLOR_BGR2RGB))
        # # Plot the lines
        # if plot_lines:
        #     for line in self.lines:
        #         plt.plot((line[0], line[2]), (line[1], line[3]), 'r-', linewidth=1)
        #     plt.show()
        self.find_vps(self.__img_path)

    def __detect_lines_in_box(self, lines, box):
        lines_inside_box = []
        lines_outside_box = []
        max_x, max_y = max(box[0], box[2]), max(box[1], box[3])
        min_x, min_y = min(box[0], box[2]), min(box[1], box[3])
        for line in lines:
            x1, y1, x2, y2 = line
            if min_x <= x1 <= max_x and min_x <= x2 <= max_x and min_y <= y1 <= max_y and min_y <= y2 <= max_y:
                lines_inside_box.append(line)
            else:
                lines_outside_box.append(line)
        self.__lines_inside_box = np.array(lines_inside_box)
        self.__lines_outside_box = np.array(lines_outside_box)
        self.__all_lines = lines

    def filter_lo_kshurim(clusters):
        stds = []
        means = []
        lines_filtered=None
        ...
        for lines, curr_std, curr_mean in zip(clusters, stds, means):
            ...
            for line in lines:
                if abs(line.m - curr_mean) < 2 * curr_std and all([abs(line.m - mean) >= 2 * std for (std, mean) in zip(other_stds, other_means)]):
                    cluster.append(line)
            if len(cluster) < 2:
                cluster = lines
    ...
    def find_vps(self, img, box = None):
        """
        ...
        if box != None:
            self.__detect_lines_in_box(self.lines, box)
        ...
        self.clusters = None  # Reset because of new image
        return best_vps
    ...
    def create_debug_VP_image(self, show_image=False, save_image=None):
        ...
        all_clusters = np.hstack(self.clusters)
        status = np.ones(self.lines.shape[0], dtype=bool)
        status[all_clusters] = False
        ind = np.where(status)[0]
        for (x1, y1, x2, y2) in self.lines[ind]:
            cv2.line(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 0), 2,
                     cv2.LINE_AA)

        # For each cluster of lines, draw them in their right colour
        for i in range(3):
            for (x1, y1, x2, y2) in self.lines[self.clusters[i]]:
